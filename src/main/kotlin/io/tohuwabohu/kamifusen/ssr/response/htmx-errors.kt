...
import io.tohuwabohu.kamifusen.ssr.renderError
...
fun createHtmxErrorResponse(status: Response.Status): Uni<Response> {
    return Uni.createFrom().item(Response.status(Response.Status.PARTIAL_CONTENT).entity(renderError { status }).build())
}
